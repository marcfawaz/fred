...
from .models import GraphAgentDefinition, ReActAgentDefinition
...
def _react_input_from_state(state: Any) -> ReActInput:
    ...
    for message in raw_messages:
        messages.append(_react_message_from_langchain(cast(AnyMessage, message)))
    return ReActInput(messages=tuple(messages))
...
def _react_message_from_langchain(message: AnyMessage) -> ReActMessage:
