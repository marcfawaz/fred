...
"""Format-agnostic inline-Markdown parser used by the docx export (#1905 port).

This is the single source of truth for the inline-Markdown SUBSET the writable
document's Word export understands: ``**bold**``, ``*italic*`` / ``_italic_``,
``***both***`` and ``` `code` ``. It is pure (no docx import) so the grammar —
which markers mean what, how strays are stripped — lives in one place; the docx
writer only owns the thin step of turning the parsed :class:`Span` list into runs.

Ported near-verbatim from Kea's ``core/markdown/inline.py`` (the module Kea shared
between its Word export and PPT filler). On Swift the PPT filler is a separate
out-of-tree capability with its own copy, so this file carries only the writable
document's needs.
...
class Span:
    ...
    ``bold`` / ``italic`` are the styles the document writer applies. ``code`` flags
    an inline-code span, which the docx export maps to a Courier font swap. A plain
    (un-emphasised) run is a span with all flags ``False``.
    """
...
def parse_inline_markdown(text: str) -> List[Span]:
    ...
    The grammar is ``**bold**``, ``*italic*`` / ``_italic_``, ``***both***`` (read as
    bold containing a stripped italic marker, i.e. bold) and ``` `code` ``. Parsing is
    flat; stray/nested markers are stripped from emphasised and plain text alike (never
    from code spans).
