...
- [x] **AUTHZ-05 review, 2026-07-09/10 (post-implementation pass triggered by human review of PR #1957).** All code items are now closed, including former item 8b; the evidence and decisions remain below and in the authorization RFC:
...
- [x] **Live-campaign findings, 2026-07-11 (items 10-11, found while standing up the `fred-deployment-factory` validation campaign):**
...
- [x] **Live-campaign findings, 2026-07-11 continued (items 12-16, found running the manual persona-by-persona pass against the live `fred-deployment-factory` stack):**
...
- Development bootstrap proven live from scratch: register `alice` in Keycloak,
  ...
  continue into Fred.
- Follow-up execution: branch
  `1989-follow-up-review-and-consolidation-of-candidate-release`.
...
- [ ] **Step 6 — v2.1.1 follow-up review, convergence and cleanup.** This is the
      canonical work plan for the release audit and replaces root-level review
      notes. Work one logical change at a time, in this order:
  - [x] **Tracking convergence:** move the still-relevant AUTHZ and ingestion
  ...
  - [ ] **Control Plane migration default — release gate:** remove the stale
        ...
        `platformbootstrap`; test the fresh-install contract.
  - [ ] **Durable import execution:** replace FastAPI-process-local
        `BackgroundTasks` execution with the existing durable executor/task
        binding and recovery model. Persist a recoverable work reference and
        prove restart, retry, cancellation and stale-task reconciliation.
  - [ ] **Provisioning convergence semantics:** either remove stale
        import-managed team/platform role tuples so the accepted bundle is the
        target state, or strictly enforce and document an empty-target-only,
        additive provisioning contract. Prove that removing an administrator
        role from the accepted input cannot silently leave the grant behind.
  - [ ] **Contract and vocabulary cleanup:** correct the import route/OpenAPI
        description (identity creation and fail-closed behavior), converge bundle
        role names with canonical `platform_admin`/`platform_observer`, regenerate
        the frontend client, and remove superseded aliases/documentation.
  - [ ] **Minimality and dead-code sweep:** trace remaining legacy bootstrap,
        authorization, import, generated-client and task-UI paths to real consumers;
        delete proven-unused paths instead of adding compatibility wrappers. Every
        retained transition must have an owner and an explicit removal condition.
  - [ ] **Bootstrap UI hardening:** make the page responsive, use semantic form
        submission, connect help/error text through accessible attributes and live
        regions, and add direct component coverage for success, 403, 409 and 503.
  - [ ] **Terminal refresh convergence:** invalidate/refetch the team list when a
        successful provisioning task reaches terminal state; prove an already-open
        Teams page updates without a full reload or a parallel state store.
  - [ ] **Operational security decision:** decide the production requirement for
        rate limiting and failed-attempt audit events, plus the explicit
        break-glass procedure if the last `platform_admin` is lost. Never log the
        submitted secret.
  - [ ] **Platform rollout decisions:** document how migration targets obtain the
        first administrator and select the AKS/TDP secret source while preserving
        the existing portable env/file application contract.
  ...
  - Cross-platform hardening review candidates (2026-07-15, ahead of the GKE
    C1 fresh-install redeploy and a joint review with Benjamin (AKS/TDP) and
    Sébastien/Arthur (S3NS/GKE C3)) — read the current implementation
    end-to-end (route, service, store, tests) and judged good enough to
    proceed with today's redeploy; not fixes, recorded so they aren't
    re-discovered independently per platform:
    1. No rate-limiting and no audit trail on failed `/bootstrap/platform-admin`
       attempts — only a successful bootstrap logs anything
       (`bootstrap/service.py`); a wrong-token guess leaves no trace. Low risk
       today (the secret is expected to be a 256-bit random value), but S3NS's
       compliance bar may require failed-attempt logging that doesn't exist
       anywhere in this service yet (ties into RFC §24.8's "no durable
       audit-log store exists yet", already tracked below).
    2. No break-glass path if the last `platform_admin` is ever lost — RFC
       Part 8 §41 scopes this out explicitly as a non-goal, but it hasn't been
       revisited for a production/S3NS context where losing the only admin is
       a real operational risk, not just a test scenario.
    3. The bootstrap design targets fresh installs only (RFC §41). S3NS is a
       Kea→Swift *migration* target, not a fresh install — confirm with
       Sébastien whether S3NS's first `platform_admin` comes through this same
       endpoint (post identity-migration, pre data-import) or needs a
       different path; not yet answered anywhere in the RFC or this backlog.
    4. Secret transport is a platform knob, not hardcoded: GKE supplies
       `bootstrap_token_env_var` from a Kubernetes Secret
       (`optional: false`); an AKS/TDP overlay via GitLab needs its own
       decision (Key Vault, sealed secret, or equivalent) before Benjamin's
       side can reuse the same `bootstrap_token_env_var`/`bootstrap_token_file`
       contract — the contract is documented (`deploy/README.md`) but no
       concrete AKS secret source has been chosen yet.
...
- [x] **Item 8b fully and finally closed (AUTHZ-08, 2026-07-20/21, issue #2034).** The identity-only guard shipped above was never generalized past `agent_app.py` — every other consumer of a personal `team_id` (filesystem/corpus/tag routes, `openai_compat_router.py`, `tasks/authz.py`, evaluations API) still 500'd or wrongly 403'd the space's own owner, and the guard shape structurally couldn't support "list my teams" enumeration. Replaced with a real, self-provisioned, write-guarded ReBAC tuple — see `id-legend.yaml` `AUTHZ-08` for the full history and `REBAC.md` § "Personal teams" for the current design. This is the closing entry for this watch item; do not re-litigate the identity-guard approach — it was tried and superseded.
